# Rule 1 - 每輪只鎖定一支仍紅 Scenario

- Level: `MUST`
- 內部迴圈每輪只鎖定**一支**仍紅的 Scenario／對應測作為實作目標（對齊「一次只過一個失敗測試」；不是 triangulation／mini-step）。
- 禁止以「一次實作讓多支仍紅 Scenario 變綠」作為推進策略；若實作後其他測附帶變綠，可計入已綠並回報，但不得當成推進策略，也不得藉此超寫；下一輪仍只挑一支仍紅者。
- 本條管**推進範圍**（每輪一支）；「寫多少」以 Rule 4（just enough）為準，不可拿「附帶變綠」當超寫藉口。
- 禁止用放寬斷言、刪測、或改 Gherkin 語意來製造綠燈。

## Good Example

- 這個例子是好的，因為本輪只鎖定 S-1-1，just enough 實作後 S-1-3 附帶 PASS 僅計入已綠，下一輪改鎖 S-1-2。

```text
本輪目標：S-1-1
→ 實作 POST /albums（just enough 讓 S-1-1 PASS）
→ 全套跑測：S-1-1 PASS；S-1-3 附帶 PASS（計入已綠）；S-1-2 仍 FAIL
→ 下一輪目標：S-1-2
```

## Bad Example

- 這個例子是壞的，因為刻意以多支同綠為推進策略，一次實作鎖定 S-1-1 與 S-1-3。

```text
仍紅：S-1-1、S-1-3
→ 刻意一次實作 POST /albums 同時打 S-1-1 與 S-1-3（以多支同綠為推進策略）
```
...
- 本輪鎖定的目標 Scenario 對應測應變綠；本層在進入本 US Green 前已綠的測試不得被弄紅。
...
- Level: `MUST`
- 每次實作必須是 `just enough code to pass test`：只寫讓**本輪鎖定的那支仍紅測**通過所需的最小程式碼，不多寫。
- 禁止超寫，至少包含：
...
- 這個例子是好的，因為只落地讓本輪目標 S-1-1 過的行為；S-1-3 若附帶 PASS 僅計入已綠，未順便做下一支仍紅或抽層。

```text
本輪目標：S-1-1
→ 實作 POST /albums（just enough 讓 S-1-1 PASS）
→ S-1-1 PASS；若 S-1-3 附帶 PASS 僅計入已綠；不做 PATCH、不抽 AlbumService
```
