...
1. READ 讀取 `rules/產物結構與章節判準.md`、`rules/實作意圖撰寫判準.md`，確認章節階層、US 節奏與實作計畫寫法。
2. THINK 依本次已載入規則，分別收斂後端／前端／整合：規格閱讀清單、環境建立步驟、各 US 的 AC／Edge、Red／Green／Refactor 巢狀實作計畫（每個 US 的 Red 區塊末尾含固定驗紅項）、進度總覽與假設；Scenario 集合對齊 `e2e-test-plan.md` 對應 `##` 區塊。
...
1. DELEGATE 執行 `uv run .agents/skills/task-plan/scripts/validate_task_plan_output.py --package specs/<NNN-plan-package>`，檢查三檔存在、必要章節、US／Red／Green／Refactor 結構、Scenario 巢狀實作計畫、每個 US Red 末尾固定驗紅項，以及 Red Scenario 與 `e2e-test-plan.md` 各層對齊。
