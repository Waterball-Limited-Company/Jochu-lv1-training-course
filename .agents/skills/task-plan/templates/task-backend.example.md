...
- [ ] `/tdd-e2e-refactor` — 行為不變（全綠）下整理本 US：去重 albums／photos 路由與資料存取、對齊錯誤形狀、必要時抽清上傳／路徑組裝（不改契約）
...
- [ ] `/tdd-e2e-refactor` — 行為不變下整理分組組裝與列表查詢
...
- [ ] `/tdd-e2e-refactor` — 行為不變下整理 reorder 與列表讀取一致性
...
- [ ] `/tdd-e2e-refactor` — 行為不變下整理列表／媒體 URL 組裝
