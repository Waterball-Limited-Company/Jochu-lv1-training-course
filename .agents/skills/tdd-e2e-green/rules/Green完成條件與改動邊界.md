...
- 本條管**推進範圍**（可以一次綠多支）；「寫多少」以 Rule 4（just enough）為準，不可拿「一次多支變綠」當超寫藉口。
...
- Level: `MUST`
- 每次實作必須是 `just enough code to pass test`：只寫讓**當下目標仍紅測**（可含同批被帶綠者）通過所需的最小程式碼，不多寫。
- 禁止超寫，至少包含：
...
- 這個例子是好的，因為只落地讓仍紅測過的行為，同批被帶綠可接受，未順便做下一 US 或抽層。

```text
仍紅：S-1-1、S-1-3
→ 實作 POST /albums（含空相簿可列表）
→ 兩支 PASS；不做 PATCH、不抽 AlbumService
```
