...
5. 對應欄位分類（必填固定；可選 **Data**）：
   - 後端：US／AC / Edge／FR／API；（若已讀 data-plan／DDL 且 Scenario 涉及可測持久化約束）另加 **Data**
   - 前端：US／AC / Edge／FR／UI
   - 整合：US／AC / Edge／FR／前端／後端；（同上條件）可另加 **Data**
6. `{{*_AC_EDGE_LINES}}`／`{{*_FR_LINES}}`／`{{*_API_LINES}}`／`{{*_UI_LINES}}`／`{{*_FRONTEND_LINES}}`／`{{*_BACKEND_LINES}}`／（可選）`{{*_DATA_LINES}}`：各填 `  - ` 開頭的條列（可多行）；無對應 FR 時可寫「（無獨立 FR 編號；…）」。
...
11. `{{ASSUMPTION_LINES}}`：`- ` 條列，掛在檔案最下方；交代證明區塊落點規則、Gherkin 慣例、與上游 spec／api-plan／ui-plan／（若有）data-plan／DDL 對齊方式等。
-->
