...
- 例外：若 `.constitution/data-plan.md`（或 `core`）以 MUST 要求某類欄位（例如每表 `created_at`／`updated_at`／`deleted_at`），該要求優先於本條「spec 未要求則不准預留」；此時 MUST 依憲法寫入，不得以本條為由省略。

## Good Example

- 這個例子是好的，因為用缺邊＋設計脈絡落實禁止巢狀，且仍保留憲法要求的稽核欄位。

```md
設計脈絡：因為相簿不可巢狀（FR-003），所以不建立父子邊、不提供 parent_album_id。
實體關聯設計／DDL：無 parent 關聯與欄位；每表仍有 created_at／updated_at／deleted_at（憲法要求）。
```

## Bad Example

- 這個例子是壞的，因為預留了被禁止的結構，或省略憲法強制的軟刪欄位。

```md
albums.parent_album_id NULL -- 以後再做巢狀
-- 或以「spec 沒寫軟刪」為由省略 deleted_at
```
