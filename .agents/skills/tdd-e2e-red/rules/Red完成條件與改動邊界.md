...
# Rule 2 - 可寫測試基建，不可寫產品業務碼

- Level: `MUST`
- 可新增／調整：E2E 測試檔、測試用 fixture（如假圖片）、測試 helper（清 DB、啟動測試 app、請求封裝）。
- 不可實作產品業務行為（例如真正處理建立相簿、上傳照片、拖放排序、寫入業務規則的 handler／UI 邏輯）；該工作留給 `/tdd-e2e-green`。
...
- 這個例子是壞的，因為在 Red 階段實作了 POST /albums 的業務 handler。

```text
在 routes/albums.js 寫入建立相簿並存 DB 的完整邏輯
```

# Rule 3 - 最小空殼僅限無業務行為；更大環境洞交回 implement

- Level: `MUST`
- 若測試因缺少可 import／啟動的入口而跑不到業務斷言，僅可補「無業務行為」的最小空殼（例如可 import 的 app，未實作的 route 回 404 或空回應）。
- 環境／基建更大缺口（依賴未裝、測試腳本不存在、目錄未建立、服務無法啟動等）必須停止，回報 implement 補洞，不可由本 skill 擴大修環境。
- 不可為了讓測試變紅或變綠而實作 Scenario 要求的行為。

## Good Example

- 這個例子是好的，因為空殼無行為，業務斷言仍紅。
...
- 禁止為製造失敗而刪改產品碼或故意破壞環境。
- 若失敗原因是測試語法錯、路徑錯、import 錯等「測試本身不可執行」，必須先修好並重跑，直到能證明是業務尚未實作導致的 Red（或改判為環境洞交回 implement）。
