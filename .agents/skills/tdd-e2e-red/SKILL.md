...
1. READ 讀取 implement 帶入的 `layer`、`plan-package`、Scenario ID／標題與受測行為。
2. READ 讀取 `rules/呼叫輸入與層別契約.md`，確認必填欄位、單 Scenario 邊界、定位與缺欄時的停止條件。
3. THINK 依本次已載入規則校驗輸入；若不通過，停止並進入 Phase 6 回報。
4. READ 依本次已載入規則，在 `specs/<plan-package>/e2e-test-plan.md` 定位該 Scenario 區塊；若依契約仍無法定位，停止並進入 Phase 6 回報。
